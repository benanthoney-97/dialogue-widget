...
export default function DialogueBarRedirect({
  agentId,
  useSignedUrl = true,
  serverLocation = "us",
  buttonColor = "#525fe1",
  ...
  const triggerRedirect = useCallback(
    (rawUrl: string | null) => {
      ...
      if (!url) return;
      if (redirectHandledRef.current && lastRedirectUrlRef.current === url) return;
  ...
  const { startSession, endSession, status, sendUserActivity, sendUserMessage } =
    useConversation({
      ...
      onDebug: (event: any) => {
        const redirectUrl = extractRedirectUrl(event);
        if (!redirectUrl) return;
        triggerRedirect(redirectUrl);
      },
      clientTools: {
        redirectToExternalURL: async (payload: any) => {
          const url = parseUrlFromArguments(payload);
          if (!url) return;
          triggerRedirect(url);
        },
      },
...
function extractRedirectUrl(event: any): string | null {
  if (!event || typeof event !== "object") return null;

  const potentialContainers = [
    event?.client_tool_call_event,
  ...
  for (const container of potentialContainers) {
    ...
    if (name !== "redirectToExternalURL") continue;

    const rawArgs =
      container?.arguments_json ??
      container?.arguments ??
    ...
    const url = parseUrlFromArguments(rawArgs);
    if (url) return url;
  }
...
function parseUrlFromArguments(args: any): string | null {
  if (!args) return null;

  if (typeof args === "string") {
    try {
      const parsed = JSON.parse(args);
      return parseUrlFromArguments(parsed);
    } catch (error) {
      return null;
    }
  }
  ...
  if (typeof args.constant_value === "string" && args.constant_value.trim())
    return args.constant_value.trim();

  if (Array.isArray(args.parameters)) {
    for (const param of args.parameters) {
      const fromParam = parseUrlFromArguments(param);
      if (fromParam) return fromParam;
    }
  }

  if (typeof args.parameters === "object") {
    const fromNested = parseUrlFromArguments(args.parameters);
    if (fromNested) return fromNested;
  }

  if (typeof args.value === "object" || typeof args.value === "string") {
    const fromValue = parseUrlFromArguments(args.value);
