...
console.log("[PrepAgentDebug] module evaluate");
...
type Props = {
  ...
  /** @deprecated Use showVoiceControls instead */
...
export default function PrepAgentDebug(props: Props) {
  const {
  ...
  console.log("[PrepAgentDebug] component render start", { agentId });
  console.log("[PrepAgentDebug] render props snapshot", {
    agentId,
    personaName,
    talkLabel,
    subtitle,
  ...
  useEffect(() => {
    console.log("[PrepAgentDebug] theme state updated", theme);
  }, [theme]);
  ...
  useEffect(() => {
    console.log("[PrepAgentDebug] agentMap state updated", agentMap);
  }, [agentMap]);
  useEffect(() => {
    console.log("[PrepAgentDebug] Persona names", {
  ...
  const displayPersonaName = useMemo(() => {
    ...
    const fallbackName =
      typeof agentMap?.agent_name === "string" ? agentMap.agent_name.trim() : "";
    return fallbackName;
  }, [personaName, agentMap?.agent_name]);

  useEffect(() => {
    console.log("[PrepAgentDebug] Display persona name", displayPersonaName);
  }, [displayPersonaName]);

  const personaImageUrl = useMemo(() => {
    const fromProp = typeof profileImage === "string" ? profileImage.trim() : "";
    if (fromProp) return fromProp;
    const fromAgentMap =
      typeof agentMap?.profile_image === "string" ? agentMap.profile_image.trim() : "";
    if (fromAgentMap) return fromAgentMap;
    return "";
  }, [profileImage, agentMap?.profile_image]);

  const resolvedPersonaImageUrl = personaImageUrl.length > 0 ? personaImageUrl : null;

  useEffect(() => {
    console.log("[PrepAgentDebug] Persona profile image", {
  ...
  useEffect(() => {
    console.log("[PrepAgentDebug] showVoiceControls prop updated", {
      showVoiceControls,
      allowVoiceSelection,
  ...
  const voiceCardContent = !resolvedShowVoiceControls ? null : (
  ...
  const containerTopOffset = isNarrow ? 36 : 80;
  ...
  const leftColumnGap = contactVisible || summaryVisible || hasDocument ? (hasDocument ? 20 : 24) : 16;
  const leftColumnStyle: CSSProperties = {
    flex: "1 1 auto",
    display: "flex",
    flexDirection: "column",
    justifyContent: hasDocument ? "space-between" : "flex-end",
    alignItems: "stretch",
    gap: leftColumnGap,
    minHeight: 0,
    paddingBottom: isNarrow ? 28 : 44,
  };
        ...
        >
          <div style={contentLayoutStyle}>
            <div style={leftColumnStyle}>
              {resolvedPersonaImageUrl ? (
                <div
                  style={{
                    alignSelf: hasDocument ? "stretch" : "center",
                    display: "flex",
                    justifyContent: hasDocument ? "flex-start" : "center",
                    marginBottom: 16,
                  }}
                >
                  <img
                    src={resolvedPersonaImageUrl}
                    alt={
                      displayPersonaName
                        ? `${displayPersonaName} profile portrait`
              ...
              >
                ...
                >
                  {displayPersonaName || "Persona placeholder not available"}
                </span>
                {!displayPersonaName ? (
                  <span
                    style={{
                      fontSize: 12,
                      lineHeight: 1.4,
                      color: "rgba(11,31,82,0.72)",
                    }}
                  >
                    Provide a personaName prop or agent_map agent_name to populate this label.
                  </span>
                ) : null}
              </div>
              {title ? (
              ...
              <div
                style={{
                  ...
                  display: contactVisible ? "block" : "none",
                }}
              ...
              <div
                style={{
                  ...
                  display: summaryVisible ? "block" : "none",
                }}
                    ...
                    <div style={{ display: "inline-flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
                      ...
                      >
                        {phase === "connecting" ? (
                          ...
                          <>
                            <svg width={isNarrow ? 16 : 22} height={isNarrow ? 16 : 22} viewBox="0 0 20 20" aria-hidden="true">
